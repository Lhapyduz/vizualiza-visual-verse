...
const ProjectForm = ({
  ...
  return <form onSubmit={handleSubmit} className="space-y-6">
      ...
      }))} className="bg-white/5 border-white/20 text-white" required />
      </div>
      ...
      }))} className="bg-white/5 border-white/20 text-white min-h-[100px]" required />
      </div>
      ...
      <div>
        ...
        category: value
          ...
          <SelectContent className="bg-vizualiza-bg-dark border-white/20">
            {categories.map(category => <SelectItem key={category.id} value={category.name} className="text-white hover:bg-white/10">
                {category.name}
              </SelectItem>)}
          </SelectContent>
        </Select>
      ...
      <TagManager tags={formData.tags} onChange={tags => setFormData(prev => ({
      ...prev,
      tags
    }))} placeholder="Digite uma tag..." maxTags={8} />

      <div>
        <label className="block text-sm font-medium text-white mb-2">Imagem Principal</label>
        <ImageUpload onImagesSelected={handleFeaturedImageUpload} maxImages={1} existingImages={formData.featured_image ? [formData.featured_image] : []} bucket="project-images" />
      </div>

      <div>
        <label className="block text-sm font-medium text-white mb-2">Imagens Adicionais</label>
        <ImageUpload onImagesSelected={handleAdditionalImagesUpload} maxImages={9} existingImages={formData.images || []} bucket="project-images" />
      </div>
      ...
      <div className="flex gap-4">
        ...
        <Button type="button" variant="outline" onClick={onCancel} className="border-white/20 text-white bg-gray-800 hover:bg-gray-700">
          Cancelar
        </Button>
      </div>
    </form>;
